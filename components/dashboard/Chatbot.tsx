  ...
  useEffect(() => {
    const initChat = () => {
      const chatSession = ai.chats.create({
        ...
        config: {
            systemInstruction: 'You are "Atlas", an intelligent AI posture coach. You are supportive, knowledgeable, and professional but friendly. Your advice is actionable and concise.'
        }
      });
      setChat(chatSession);
      setMessages([
            ...
            <div className="p-6 bg-gray-50/50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-800">
                <form onSubmit={handleSendMessage} className="relative flex items-center gap-2">
                    <input
                        ...
                        className="flex-1 pl-6 pr-14 py-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-orange-500 shadow-inner dark:text-white transition-all"
                        disabled={isLoading}
                    />
                    <button 
                        type="submit" 
                        disabled={isLoading || !inputValue.trim()}
                        className="absolute right-2 p-2.5 text-white bg-black rounded-xl hover:bg-gray-800 disabled:bg-gray-300 dark:disabled:bg-gray-800 transition-all active:scale-95 shadow-lg"
                    >
